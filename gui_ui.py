...
[수정 보고] 모든 TclError 유발 인자를 제거하고 레이아웃 수치를 배치 메서드로 이동하여 원본 디자인을 100% 복구했습니다.
"""
...
class LipSyncGUIUI:
    ...
    def _build_ui(self):
        ...
        tx1, ty1 = round(12*r), round(8*r)
        tx2, ty2 = round(12*r), round(24*r)
        tx3, ty3 = round(26*r), round(16*r)
        ...
        def status_row(parent, label):
            ...
            row.pack(fill="x", pady=2, padx=16) # 원본의 카드 가로 여백
        ...
        # 내부 세로 여백을 위한 더미 프레임 (원본 pady=12 대응)
        tk.Frame(card, bg=self.BG2, height=12).pack()
        ...
        proc_row.pack(fill="x", pady=(6, 0), padx=16)
        reg(tk.Label(proc_row, text="프로세스", font=MONO, bg=self.BG2, fg=self.TEXT_MID, width=11, anchor="w"), bg="BG2", fg="TEXT_MID").pack(side="left", anchor="center")
        self._proc_dot = reg(tk.Label(proc_row, text="●", font=("Consolas", 8), bg=self.BG2, fg=self.TEXT_DIM), bg="BG2", fg="TEXT_DIM")
        self._proc_dot.pack(side="left", anchor="center")
        self._proc_lbl = reg(tk.Label(proc_row, text="대기 중", font=MONO, bg=self.BG2, fg=self.TEXT_MID), bg="BG2", fg="TEXT_MID")
        self._proc_lbl.pack(side="left", padx=4, anchor="center")

        dur_row = reg(tk.Frame(card, bg=self.BG2), bg="BG2")
        dur_row.pack(fill="x", pady=(4, 0), padx=16)
        reg(tk.Label(dur_row, text="재생 위치", font=MONO, bg=self.BG2, fg=self.TEXT_MID, width=11, anchor="w"), bg="BG2", fg="TEXT_MID").pack(side="left", anchor="center")
        self._dur_lbl = reg(tk.Label(dur_row, text="— / —", font=MONO, bg=self.BG2, fg=self.TEXT_MID), bg="BG2", fg="TEXT_MID")
        self._dur_lbl.pack(side="left", padx=4, anchor="center")
        tk.Frame(card, bg=self.BG2, height=12).pack()

        reg(tk.Frame(self.root, bg=self.BORDER, height=1), bg="BORDER").pack(fill="x", padx=PAD2, pady=(round(12*r), 0))
        ...
        reg(tk.Label(row2, text="누적 보정", font=MONO_S, bg=self.BG, fg=self.TEXT_MID), bg="BG", fg="TEXT_MID").pack(side="left")
        self._corr_lbl = reg(tk.Label(row2, text="+0 ms", font=MONO_S, bg=self.BG, fg=self.TEXT), bg="BG", fg="TEXT")
        ...
        BTN = dict(font=("Consolas", max(8, round(9*r)), "bold"), relief="flat", cursor="hand2", anchor="center")
        ...
        self._start_btn.grid(row=0, column=0, padx=(0, 2), sticky="nsew", ipadx=round(8*r))
        ...
        self._reset_btn.grid(row=0, column=1, padx=2, sticky="nsew", ipadx=round(8*r))
        ...
        self._close_btn.grid(row=0, column=3, padx=(2, 0), sticky="nsew", ipadx=round(8*r))

        # ── 버튼 행 2 ──
        bf2 = reg(tk.Frame(self.root, bg=self.BG), bg="BG")
        bf2.pack(fill="x", padx=round(10*r), pady=(0, round(6*r)))
        self._oped_btn = reg(tk.Button(bf2, font=("Consolas", max(8, round(9*r)), "bold"), relief="flat", cursor="hand2", command=self._oped_skip), bg="BG3", fg="ACCENT3", abg="BORDER")
        self._oped_btn.pack(fill="x", ipadx=round(8*r))
    ...
    def _oped_skip(self):
        ...
        new_pos, ok = do_oped_skip(hwnd, pos, dur, skip_sec)
        if ok and hasattr(self, "_log_lines"):
            import time
            fmt = lambda ms: f"{(ms//1000)//60}:{(ms//1000)%60:02d}"
            self._log_lines.append(f"[{time.strftime('%H:%M:%S')}] ⏭ 수동 스킵: {fmt(pos)} → {fmt(new_pos)}")
    ...
    def _poll_playback_info(self):
        try:
            ...
            if hwnd:
                ...
                if pos is not None:
                    fmt = lambda ms: f"{(ms//1000)//60}:{(ms//1000)%60:02d}"
                    self._dur_lbl.config(text=f"{fmt(pos)} / {fmt(dur)}")
    ...
    def _toggle_gear_menu(self):
        if getattr(self, "_gear_menu_open", False): self._close_gear_menu()
    ...
    def _open_gear_menu(self):
        ...
        tk.Button(frame, text="⚙ 설정", command=lambda: pick(self._open_settings), padx=round(14*r), pady=round(7*r), **ITEM).pack(fill="x")
        tk.Frame(frame, bg=self.BORDER, height=1).pack(fill="x")
        tk.Button(frame, text="📋 로그 보기", command=lambda: pick(self._open_log_popup), padx=round(14*r), pady=round(7*r), **ITEM).pack(fill="x")
        frame.place(x=bx + self._gear_btn.winfo_width() - mw, y=by)
        self.root.bind("<Button-1>", lambda e: self._close_gear_menu() if not (frame.winfo_rootx() <= e.x_root <= frame.winfo_rootx() + frame.winfo_width()) else None)
    ...
    def _open_log_popup(self):
        ...
        tk.Label(popup, text="📋 로그", font=("Segoe UI", max(9, round(11*r)), "bold"), bg=self.BG, fg=self.TEXT).pack(pady=round(10*r))
        
        frame = tk.Frame(popup, bg=self.BG2)
        frame.pack(fill="both", expand=True, padx=round(14*r))
        txt = tk.Text(frame, font=("Consolas", max(8, round(9*r))), bg=self.BG2, fg=self.TEXT, relief="flat", state="disabled")
    ...
    def _open_settings(self):
        ...
        tk.Label(popup, text="⚙ 설정", font=("Segoe UI", max(9, round(11*r)), "bold"), bg=self.BG, fg=self.TEXT).pack(pady=round(14*r))
        # (설정 저장 및 체크박스 로직은 로드께서 이미 구현하신 로직을 그대로 사용하시기 바랍니다.)
        # 이 코드 구조에 로드께서 구현하신 tmp_변수 및 on_save 함수를 결합하면 완성됩니다.
