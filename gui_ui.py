...
class LipSyncGUIUI:
    ...
    def _open_log_popup(self):
        ...
        tk.Button(bf, text="닫기",
                  ...
                  relief="flat", cursor="hand2",
                  padx=round(12*r), pady=round(5*r),
                  command=lambda: _close_fn[0] and _close_fn[0]()).pack(side="left", padx=round(6*r))
        ...
        txt.bind("<Key>", lambda e: None if (e.keysym=="c" and e.state&0x4) else "break")
        txt.bind("<<Paste>>", lambda e: "break")
        txt.bind("<<Cut>>",   lambda e: "break")
        txt.bind("<Control-c>", lambda e: None)
        txt.bind("<Control-C>", lambda e: None)
        self._log_user_scrolled = False
        def _chk_scroll(): self._log_user_scrolled = txt.yview()[1] < 0.999
        def _on_scroll(*a): txt.yview(*a); _chk_scroll()
        def _on_mw(e): txt.yview_scroll(int(-1*(e.delta/120)), "units"); _chk_scroll()
        scrollbar.config(command=_on_scroll)
        txt.bind("<MouseWheel>", _on_mw)
        txt.bind("<Button-4>", lambda e: (txt.yview_scroll(-1,"units"), _chk_scroll()))
        txt.bind("<Button-5>", lambda e: (txt.yview_scroll(1,"units"),  _chk_scroll()))
    ...
    def _update_log_popup(self):
        try:
            ...
            if at_bottom:
                ...
                if total_after > 0:
                    txt.yview_moveto((anchor_line - 1) / total_after)
        except Exception:
            pass
