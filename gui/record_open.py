...
class LipSyncGUIRecordOpen:
    ...
    def _open_record_capture(self):
        ...
        def show_recording_overlay():
            from gui.record_backend import _show_overlay, _active_overlays
            # 기존 녹화 오버레이가 남아있으면 먼저 닫기
            close_recording_overlay()
            _show_overlay(self.root, "🔴 녹화중", duration_ms=99999999)
            # _show_overlay가 _active_overlays 마지막에 추가한 위젯을 보관
            if _active_overlays:
                state["overlay"] = _active_overlays[-1]
        ...
        def stop_record():
            ...
            def _save():
                ...
                try:
                    ...
                    # _retiming_audio() 가 드리프트를 이미 보정한 오디오를 반환하므로,
                    # 여기서는 영상 첫 프레임과 오디오 첫 청크의 시작 시각 차이만 구한다.
    ...
    # ── 녹화/캡처 탭 (메인창 탭에 직접 내장) ────────────────────────────────
    def _build_record_tab(self, parent, r, P, P2):
        """메인창 '녹화/캡처' 탭 위젯 구성. 팝업 없이 탭 프레임에 직접 배치."""
        save_dir = getattr(self, "_record_save_dir", None) or self._load_setting("record_save_dir", "")
        ...
        # ── 저장 위치 ──────────────────────────────────────────────────────────
        dir_card = tk.Frame(parent, bg=self.BG2, padx=PAD2, pady=PAD_V)
        dir_card.pack(fill="x", padx=P2, pady=(round(10*r), 0))
        tk.Label(dir_card, text="저장 위치",
                 font=("Consolas", F_MONO, "bold"),
                 bg=self.BG2, fg=self.TEXT_MID).pack(anchor="w")
        dir_row = tk.Frame(dir_card, bg=self.BG2)
        ...
        tk.Frame(parent, bg=self.BORDER, height=1).pack(fill="x", padx=P2, pady=(PAD_V, 0))

        # ── 내부 탭 (녹화 / 캡처) ──────────────────────────────────────────────
        tab_btn_f   = tk.Frame(parent, bg=self.BG)
        tab_btn_f.pack(fill="x", padx=P2, pady=(PAD_V, 0))
        tab_content = tk.Frame(parent, bg=self.BG2)
        tab_content.pack(fill="both", expand=True, padx=P2, pady=(0, PAD_V))
        ...
        range_row = tk.Frame(record_page, bg=self.BG2)
        range_row.pack(anchor="w", pady=(0, round(8*r)))
        vcmd = (parent.register(lambda s: all(c.isdigit() or c == ":" for c in s) or s == ""), "%P")
