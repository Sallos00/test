...
# ── 오디오 청크 스트리밍 I/O 헬퍼 ─────────────────────────────────────────────
# 포맷: [qpc_sec: double 8B][data_bytes: uint32 4B][float32 PCM data: data_bytes B] 반복
# 이 헬퍼를 통해 녹화 중 오디오 청크를 메모리 대신 임시 파일에 직접 기록한다.

def _write_audio_chunk(fh, lock, qpc_sec, float32_arr):
    """float32 numpy 배열 청크를 임시 파일에 append 방식으로 기록한다.
    fh: 열린 파일 핸들 (wb 모드), lock: threading.Lock.
    실패 시 조용히 무시 (호출자가 메모리 폴백 여부를 판단한다)."""
    try:
...
def _read_audio_chunks_from_file(path):
    """_write_audio_chunk()로 기록한 임시 파일을 읽어 청크 리스트로 복원한다.
    반환: [(qpc_sec: float, arr: np.ndarray), ...]  — 실패 시 빈 리스트."""
    import numpy as np
    chunks = []
    try:
        with open(path, 'rb') as f:
            while True:
                header = f.read(12)            # double(8) + uint32(4)
                if len(header) < 12:
                    break
                ...
                while recorder._running:
                    ...
                    if num_frames > 0:
                        if not (flg & AUDCLNT_BUFFERFLAGS_SILENT) and data.value:
                            ...
                            # 스트리밍 모드: 파일에 쓰고 즉시 해제 → 녹화 중 힙 증가 없음
                            # 파일 쓰기 실패 시 기존 메모리 방식으로 폴백 (안전망)
...
class _AudioRecorder:
    def __init__(self):
        ...
        # ── 스트리밍 모드 상태 (녹화 중 청크를 메모리 대신 임시 파일에 기록) ──
        self._tmp_audio_path = None   # mkstemp 로 생성한 임시 파일 경로
        self._tmp_audio_fh   = None   # 쓰기용 파일 핸들 (wb)
        self._chunk_lock     = threading.Lock()  # 파일 쓰기 직렬화용
        self._stream_mode    = False  # 임시 파일 활성화 여부
    ...
    def stop(self):
        ...
        # ── 스트리밍 모드: 파일에서 청크 복원 ────────────────────────────────
        # 파일 읽기 실패 시 메모리 폴백 청크(_chunks)를 사용하므로 데이터 손실 없음
        file_chunks = []
