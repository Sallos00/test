...
class LipSyncGUIUI:
    ...
    # ── OP/ED 감지 팝업 ────────────────────────────────────────────────
    def _show_oped_skip_popup(self, oped_prompt: dict, auto_mode: bool = False):
        """
        processes.py에서 전달한 oped_prompt를 받아 OP/ED 스킵 확인 팝업을 띄운다.

        - 스킵 누르면 설정된 OPED_SKIP_SEC 만큼 이동
        - 아무것도 안 누르면 10초 후 팝업 자동 닫힘(무시로 처리)
        - 닫히는 순간 모두 3분 쿨다운 시작
        """
        try:
            if not isinstance(oped_prompt, dict):
                return
            prompt_id = oped_prompt.get("prompt_id")
            zone = oped_prompt.get("zone")
            if not prompt_id or not zone:
                return

            cur = getattr(self, "_oped_prompt_ui", None)
            if cur and cur.get("open", False):
                # prompt_pending로 중복 생성이 억제되지만, 혹시 모를 중복은 차단
                return

            self._oped_prompt_ui = {"prompt_id": prompt_id, "zone": zone, "open": True, "auto_mode": auto_mode}
            ...
            op_or_ed = "OP" if zone == "오프닝" else "ED"
            q = f"{op_or_ed}을 스킵하시겠습니까?"
            ...
            try:
                if hwnd:
                    ...
                    x = max(0, min(screen_w - pw, rect.right - pw - margin))
                    y = max(0, min(screen_h - ph, rect.bottom - ph - margin))
            ...
            F_TITLE = max(11, round(13 * r))
            F_BTN = max(9, round(10 * r))
            PAD = round(12 * r)

            tk.Label(popup, text=q,
                     font=("Segoe UI", F_TITLE, "bold"),
                     bg=self.BG, fg=self.TEXT).pack(pady=(PAD, 0))
            ...
            tk.Label(popup,
                     ...
                     font=("Consolas", max(8, round(9 * r))),
                     bg=self.BG2, fg=self.TEXT_MID,
                     justify="left").pack(fill="x", padx=round(16 * r), pady=(round(10 * r), 0))

            btn_f = tk.Frame(popup, bg=self.BG)
            btn_f.pack(fill="x", pady=round(10 * r), padx=round(16 * r))

            def _decide(decision: str):
                try:
                    qref = self._auto_cmd_queue if auto_mode else self.cmd_queue
                    qref.put_nowait((decision, prompt_id))
                except Exception:
                    pass
                ...
                try:
                    self._oped_prompt_ui["open"] = False
                except Exception:
                    pass

            def on_yes():
                _decide("oped_prompt_yes")

            def on_no():
                _decide("oped_prompt_no")

            popup.protocol("WM_DELETE_WINDOW", on_no)

            tk.Button(btn_f, text="⏭ 스킵",
                      font=("Consolas", F_BTN, "bold"),
                      bg=self.BG3, fg=self.ACCENT3,
                      activebackground=self.BORDER,
                      relief="flat", cursor="hand2",
                      padx=round(18 * r), pady=round(6 * r),
                      command=on_yes).pack(side="left", padx=round(6 * r))
            ...
            tk.Button(btn_f, text="무시",
                      ...
                      relief="flat", cursor="hand2",
                      padx=round(18 * r), pady=round(6 * r),
                      command=on_no).pack(side="left", padx=round(6 * r))

            popup.after(round(10 * 1000), on_no)

        except Exception:
            try:
                self._oped_prompt_ui = None
            except Exception:
                pass
